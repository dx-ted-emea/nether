...
namespace NetherSDK
{
    public class NetherClient : MonoBehaviour
    {
        ...
        public void GetPlayer(Action<CallbackResponse<Player>> callback)
        {
            ...
            StartCoroutine(CallNetherServiceInternal<Player>(null, GetNetherURL<Player>(), HttpMethod.Get, NetherAuthorizationToken, callback));
        }

        public void PutPlayer(Player instance, Action<CallbackResponse<Player>> callback)
        {
            Utilities.ValidateForNull(instance, callback);
            StartCoroutine(CallNetherServiceInternal<Player>(instance, GetNetherURL<Player>(), HttpMethod.Put, NetherAuthorizationToken, callback));
        }

        public void GetLeaderboards(Action<CallbackResponse<Leaderboard[]>> callback)
        {
            Utilities.ValidateForNull(callback);
            StartCoroutine(CallNetherServiceInternal<Leaderboard[]>(null, GetNetherURL<Leaderboard[]>(), HttpMethod.Get, NetherAuthorizationToken, callback));
        }
        ...
        public void GetLeaderboardNamed(string leaderboardName, Action<CallbackResponse<LeaderboardNamed>> callback)
        {
            ...
            StartCoroutine(CallNetherServiceInternal<LeaderboardNamed>(null, GetNetherURL<Leaderboard[]>() + "/" + leaderboardName,
                HttpMethod.Get, NetherAuthorizationToken, callback, false));
        }

        public void PostScore(Score instance, Action<CallbackResponse<Score>> callback)
        {
            Utilities.ValidateForNull(instance, callback);
            StartCoroutine(CallNetherServiceInternal<Score>(instance, GetNetherURL<Score>(), HttpMethod.Post, NetherAuthorizationToken, callback));
        }

        public void GetEndpoint(Action<CallbackResponse<Endpoint>> callback)
        {
            Utilities.ValidateForNull(callback);
            StartCoroutine(CallNetherServiceInternal<Endpoint>(null, GetNetherURL<Endpoint>(), HttpMethod.Get, NetherAuthorizationToken, callback, false));
        }
        ...
        private object heartBeatInstanceToSend = null;
        public void StartHeartbeat(float time, float repeatRate, object heartBeatInstanceToSend)
        {
            ...
            this.heartBeatInstanceToSend = heartBeatInstanceToSend;
        ...
        {
            PostData(heartBeatInstanceToSend, (x) =>
            {
                if (Globals.DebugFlag)
